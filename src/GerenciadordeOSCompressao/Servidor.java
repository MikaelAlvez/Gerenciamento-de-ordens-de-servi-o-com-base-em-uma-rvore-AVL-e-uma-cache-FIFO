...
public class Servidor {
    ...
    public Servidor() {
        this.BancodeDados = new TabelaHash(70);
    ...
    public String receberMensagem(MensagemComprimida mensagemComprimida) {
        // Reconstruir a árvore de Huffman a partir das frequências
        NoHuffman arvore = compressaoHuffman.construirArvore(mensagemComprimida.getFrequencias());

        // Descomprimir a mensagem
        String mensagemDescomprimida = descomprimir(mensagemComprimida.getMensagemComprimida(), arvore);
        return mensagemDescomprimida;
    }

    public String descomprimir(String mensagemComprimida, NoHuffman raiz) {
        StringBuilder mensagemDescomprimida = new StringBuilder();
        NoHuffman noAtual = raiz;

        for (int i = 0; i < mensagemComprimida.length(); i++) {
            noAtual = mensagemComprimida.charAt(i) == '0' ? noAtual.esquerdo : noAtual.direito;

            // Quando chegar numa folha, adiciona o caractere e volta para a raiz
            if (noAtual.esquerdo == null && noAtual.direito == null) {
                mensagemDescomprimida.append(noAtual.caractere);
                noAtual = raiz;
            }
        }

        return mensagemDescomprimida.toString();
    }
